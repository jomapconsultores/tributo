-- 053: Honorarios por período (mes/año)
-- (venía de backend/migrations/029, que nunca se aplicaba: la carpeta que se
...
-- Se suelta antes de crearla: `ADD CONSTRAINT` no admite IF NOT EXISTS y, sobre
-- una base que ya la tiene, la migración moriría acá.
ALTER TABLE reportes_honorarios
  DROP CONSTRAINT IF EXISTS reportes_honorarios_user_ident_prod_periodo_key;
ALTER TABLE reportes_honorarios
