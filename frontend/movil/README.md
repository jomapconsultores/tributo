# Gestor Tributario — aplicación móvil (Android e iOS)

Desarrollado por Marco Antonio Posligua San Martín.

## Qué es esto

Gestor Tributario ya funciona como **PWA**: desde el navegador del teléfono se instala
...
La app abre el despliegue en Coolify, **https://tributos.pensamiento-libre.org**, que
...
mkdir -p ~/claves
keytool -genkey -v -keystore ~/claves/tributos-web.keystore -alias tributos-web \
...
Los flujos `.github/workflows/movil-android-tributos-web.yml` y
`movil-ios-tributos-web.yml` compilan en la nube. Configura en *Settings → Secrets and variables → Actions*:

| Nombre | Tipo | Para qué |
|---|---|---|
| `APP_URL` | variable *(opcional)* | otro destino en vez del de Coolify, que ya viene fijado |
| `ANDROID_KEYSTORE_BASE64` | secreto | `base64 -w0 ~/claves/tributos-web.keystore` |
