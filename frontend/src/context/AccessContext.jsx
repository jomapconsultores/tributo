...
export function AccessProvider({ children }) {
  const [state, setState] = useState({ modules: [], isAdmin: false, role: 'cliente', subscription: null, loading: true })

  useEffect(() => {
    accessAPI.me()
      .then((r) => setState({ modules: r.data?.modules || [], isAdmin: !!r.data?.is_admin, role: r.data?.role || 'cliente', subscription: r.data?.subscription || null, loading: false }))
  ...
  return <AccessContext.Provider value={{ ...state, has, isSuperAdmin }}>{children}</AccessContext.Provider>
}
